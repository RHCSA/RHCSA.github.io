#!/bin/bash
# Objective 8: repositories
# LAB: Repo config 11
# Converted from qdir format: q011-rhcsa-08-011

IS_LAB=true
LAB_ID="rhcsa_08_011"
QUESTION="Repo config 11"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Repo config 11"
...
# Task 2
TASK_2_QUESTION="Maak /etc/yum.repos.d/rhcsa-local.repo met enabled=1."
...
[[ -f /etc/yum.repos.d/rhcsa-local.repo ]] || fail 'repo file missing'
grep -q '^enabled=1' /etc/yum.repos.d/rhcsa-local.repo || fail 'repo not enabled'
pass 'repo configured'
}
