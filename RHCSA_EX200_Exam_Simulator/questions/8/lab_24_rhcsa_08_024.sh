#!/bin/bash
# Objective 8: flatpak, reporting
# LAB: Flatpak audit 24
# Converted from qdir format: q024-rhcsa-08-024

IS_LAB=true
LAB_ID="rhcsa_08_024"
QUESTION="Flatpak audit 24"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Flatpak audit 24"
...
# Task 2
TASK_2_QUESTION="Maak /root/reports/flatpak-audit.txt met remotes en apps."
...
[[ -s /root/reports/flatpak-audit.txt ]] || fail 'flatpak audit missing'
grep -Eiq 'remote|application|flatpak|flathub' /root/reports/flatpak-audit.txt || fail 'audit incomplete'
pass 'flatpak audit exists'
}
...
cleanup_lab() {
set +e
rm -f /etc/yum.repos.d/rhcsa-local.repo /root/reports/dnf-provides-tree.txt /root/reports/flatpak-audit.txt
