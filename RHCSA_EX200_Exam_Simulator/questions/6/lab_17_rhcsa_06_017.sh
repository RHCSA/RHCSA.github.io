#!/bin/bash
# Objective 6: systemd, services
# LAB: Journald troubleshooting 17
# Converted from qdir format: q017-rhcsa-06-017

IS_LAB=true
LAB_ID="rhcsa_06_017"
QUESTION="Journald troubleshooting 17"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Journald troubleshooting 17"
...
# Task 2
TASK_2_QUESTION="Maak een journald troubleshootingrapport in /root/reports/journal-report.txt."
...
[[ -s /root/reports/journal-report.txt ]] || fail 'journal report missing'
grep -Eiq 'journal|boot|error|service|unit|failed' /root/reports/journal-report.txt || fail 'journal report incomplete'
pass 'journal report exists'
}
...
cleanup_lab() {
set +e
rm -f /etc/systemd/system/rhcsa-demo-*.service /etc/systemd/system/rhcsa-timer-*.timer /etc/systemd/system/exam-app-*.service /usr/local/bin/rhcsa-demo-*.sh
