#!/bin/bash
# Objective 4: storage, shell_scripting
# LAB: Storage audit script
# Converted from qdir format: q018-rhcsa-04-018

IS_LAB=true
LAB_ID="rhcsa_04_018"
QUESTION="Storage audit script"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Storage audit script"
...
# Task 2
TASK_2_QUESTION="Maak /usr/local/bin/storage-audit.sh dat storage rapporteert."
...
[[ -x /usr/local/bin/storage-audit.sh ]] || fail 'script missing'
/usr/local/bin/storage-audit.sh >/root/reports/storage-audit.txt
[[ -s /root/reports/storage-audit.txt ]] || fail 'report missing'
pass 'storage audit works'
}
