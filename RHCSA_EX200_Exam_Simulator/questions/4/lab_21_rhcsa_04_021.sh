#!/bin/bash
# Objective 4: bind_mount
# LAB: Create bind mount
# Converted from qdir format: q021-rhcsa-04-021

IS_LAB=true
LAB_ID="rhcsa_04_021"
QUESTION="Create bind mount"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Create bind mount"
...
# Task 2
TASK_2_QUESTION="Maak /srv/source bind-mounted naar /srv/target persistent."
...
mkdir -p /srv/source /srv/target
}

_validate_converted_lab() {
findmnt /srv/target >/dev/null || fail 'bind mount missing'
grep -E '[[:space:]]/srv/target[[:space:]].*bind' /etc/fstab >/dev/null || fail 'bind mount not persistent'
pass 'bind mount configured'
}
