...
# starts, so a container has no boot phase to test this in. No IS_LAB line,
# so both the CLI and the webui treat this as a plain Question/Answer entry
# (QUESTION + ANSWER only, no HINT needed).
