#!/bin/bash
# Objective 4: mkfs_xfs, mount
# LAB: Create XFS data directory
# Converted from qdir format: q001-rhcsa-04-001

IS_LAB=true
LAB_ID="rhcsa_04_001"
QUESTION="Create XFS data directory"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Create XFS data directory"
...
# Task 2
TASK_2_QUESTION="Maak /data aan en mount een XFS-filesystem daarop."
...
mountpoint_exists /data || fail '/data is not mounted'
findmnt -no FSTYPE /data | grep -Fx xfs >/dev/null || fail '/data is not XFS'
pass '/data mounted as XFS'
}
