#!/bin/bash
# Objective 4: label
# LAB: Recover filesystem label
# Converted from qdir format: q025-rhcsa-04-025

IS_LAB=true
LAB_ID="rhcsa_04_025"
QUESTION="Recover filesystem label"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Recover filesystem label"
...
# Task 2
TASK_2_QUESTION="Geef filesystem label DATA aan /data filesystem."
...
findmnt -no SOURCE /data >/dev/null || fail '/data not mounted'
blkid "$(findmnt -no SOURCE /data)" | grep -q 'LABEL="DATA"' || fail 'DATA label missing'
