#!/bin/bash
# Objective 4: fstab_options
# LAB: Mount options noexec
# Converted from qdir format: q015-rhcsa-04-015

IS_LAB=true
LAB_ID="rhcsa_04_015"
QUESTION="Mount options noexec"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Mount options noexec"
...
# Task 2
TASK_2_QUESTION="Mount /securedata persistent met noexec optie."
...
findmnt -no OPTIONS /securedata 2>/dev/null | grep -qw noexec || fail 'noexec not active'
grep -E '[[:space:]]/securedata[[:space:]].*noexec' /etc/fstab >/dev/null || fail 'noexec not persistent'
pass 'noexec configured'
}
