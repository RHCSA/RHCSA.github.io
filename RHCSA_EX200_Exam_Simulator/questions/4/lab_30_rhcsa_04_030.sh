#!/bin/bash
# Objective 4: storage, reporting
# LAB: Full storage report
# Converted from qdir format: q030-rhcsa-04-030

IS_LAB=true
LAB_ID="rhcsa_04_030"
QUESTION="Full storage report"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Full storage report"
...
# Task 2
TASK_2_QUESTION="Maak /root/reports/storage-full-audit.txt met disks, filesystems, LVM, swap en mounts."
...
[[ -s /root/reports/storage-full-audit.txt ]] || fail 'full audit missing'
grep -Eiq 'lsblk|lvs|vgs|swapon|mount|df|filesystem' /root/reports/storage-full-audit.txt || fail 'full audit incomplete'
pass 'full storage audit exists'
}
