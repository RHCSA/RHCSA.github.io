#!/bin/bash
# Objective 4: tar
# LAB: Archive storage configs
# Converted from qdir format: q024-rhcsa-04-024

IS_LAB=true
LAB_ID="rhcsa_04_024"
QUESTION="Archive storage configs"

LAB_TASK_COUNT=3

# Task 1
TASK_1_QUESTION="Prepare the lab environment for: Archive storage configs"
...
# Task 2
TASK_2_QUESTION="Archiveer /etc/fstab en repo storage config naar /root/reports/storage-configs.tar.gz."
...
[[ -s /root/reports/storage-configs.tar.gz ]] || fail 'archive missing'
tar -tzf /root/reports/storage-configs.tar.gz | grep -q 'fstab' || fail 'fstab not in archive'
pass 'archive valid'
}
