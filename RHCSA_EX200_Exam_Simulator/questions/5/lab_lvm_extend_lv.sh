...
# LAB: Extend an existing logical volume and grow its file system
...
STATE_FILE="/tmp/.rhcsa_lab_lvm_extend_disk"
VG_NAME="vg-to-extend"
LV_NAME="lv-to-extend"
INITIAL_SIZE="1G"
INITIAL_SIZE_BYTES=$((1 * 1024 * 1024 * 1024))
EXTEND_MB=500
FINAL_SIZE_BYTES=$((INITIAL_SIZE_BYTES + EXTEND_MB * 1024 * 1024))
MOUNT_POINT="/extend-data"
MIN_DISK_SIZE_GB=5
...
# Task 6
TASK_6_QUESTION="Extend ${LV_NAME} by +${EXTEND_MB}M and grow its file system to match, in one command"
TASK_6_HINT="Use lvextend with --resizefs so the file system is grown along with the logical volume"
TASK_6_COMMAND_1="lvextend --resizefs --size +${EXTEND_MB}M /dev/${VG_NAME}/${LV_NAME}"
...
check_tasks() {
    ...
    # (>= not == because task 5 grows it later - a strict equality check
    ...
    # Task 5: logical volume is EXACTLY the extended size AND the file system
    ...
    if lvs "${VG_NAME}/${LV_NAME}" &>/dev/null; then
        ...
        if [[ "$lv_bytes" == "$FINAL_SIZE_BYTES" ]] && [[ -n "$fs_blocks" ]] && [[ -n "$fs_bsize" ]]; then
            fs_bytes=$((fs_blocks * fs_bsize))
            if (( fs_bytes >= FINAL_SIZE_BYTES * 90 / 100 )); then
                TASK_STATUS[5]="true"
            else
                TASK_STATUS[5]="false"
            fi
